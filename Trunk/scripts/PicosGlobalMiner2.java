...
public class PicosGlobalMiner2 extends Script implements PaintListener {
    ...
    /*
      ...
      * by Pico - AKC_Pico@hotmail.com (MY ONLY MSN). 
    ...
    static final int MAX_FAILS = 25;

    // /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Int //
    // /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    public static final int miningAnimation = 624;
    int oldExp;
    int oldLevels;
    ...
    @Override
    public int loop() {
        if (player.getMyEnergy() == random(31, 56) || player.getMyEnergy() > 56) {
            game.setRun(true);
        }
        standingStill();
        ...
        if (player.getMine().getAnimation() == miningAnimation
                || player.getMine().getAnimation() == 625) {
        ...
        if (inventory.isFull()) {
            ...
            randomStuff();
            antiBan();
            inventory.dropAllExcept(picks);
            return antiBan();
        }

        final RSObject rocks = objects.getNearestByID(rock);
    ...
    public void onRepaint(final Graphics g) {
        ...
        if (startLevel == 0 || startExp == 0) {
            ...
            startExp = skills.getCurrentXP(index);
            oldExp = 0;
        }
        ...
        if (exp > oldExp) {
            ...
            oldExp = exp;
            rockCount++;
            countToNext = skills.getXPToNextLvl(Constants.STAT_MINING)
                    / xpPerRock + 1;
        }

        levels = skills.getCurrentLvl(index) - startLevel;
        if (levels > oldLevels) {
            oldLevels = levels;
        }
        ...
        g.drawString("Percent to next level: " + "%"
                + skills.getPercentToNextLvl(index), 9, 219);
    ...
    public void standingStill() {
        ...
        if (oldPosition.getX() == player.getMyLocation().getX()
                && oldPosition.getY() == player.getMyLocation().getY()) {
            standingCount++;
            // log("ADDING TO COUNT!!!");
        }
        if (oldPosition.getX() != player.getMyLocation().getX()
            ...
            oldPosition = new RSTile(player.getMyLocation().getX(),
