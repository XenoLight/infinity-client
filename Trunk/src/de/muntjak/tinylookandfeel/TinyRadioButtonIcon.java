...
class TinyRadioButtonIcon extends TinyCheckBoxIcon {
	...
	private static class DisabledRadioKey {
		...
		public boolean equals(final Object o) {
			...
			final DisabledRadioKey other = (DisabledRadioKey) o;
	...
	/*
	 * EnabledRadioKey is used as key in the cache HashMap. Overrides equals()
	 * and hashCode(). Used only if we are run from ControlPanel.
	 */
	private static class EnabledRadioKey {
		...
		public boolean equals(final Object o) {
			...
			final EnabledRadioKey other = (EnabledRadioKey) o;
	...
	private static class RadioKey {
		...
		RadioKey(final Color c, final Color background, final boolean pressed, final boolean enabled,
				final boolean rollover, final boolean focused) {
			this.c = c;
	...
	private void drawXpCheckMark(final Graphics g, final Color c, final int x, final int y) {
		g.translate(x, y);
	...
	private void drawXpRadio(final Graphics g, final AbstractButton b, Color c, final int x,
		...
		final RadioKey key = new RadioKey(c, bg, pressed, enabled,
		...
		if (TinyLookAndFeel.PRINT_CACHE_SIZES) {
			System.out
			.println("TinyRadioButtonIcon.cache.size=" + cache.size());
		}
	}

	// the old routine - execute this, if we are in the control panel
	// (else border colors not updated)
	private void drawXpRadioNoCache(final Graphics g, final AbstractButton b, Color c,
		...
		if (useCachedImage) {
			if (enabled) {
				key = new EnabledRadioKey(c, Theme.buttonBorderColor.getColor());
			} else {
		...
		if (!enabled) {
			DrawRoutines.drawXpRadioBorder(graphics,
					Theme.buttonBorderDisabledColor.getColor(), bx, by, w, h);
		} else {
			if (rollover && Theme.buttonRolloverBorder.getValue() && !pressed) {
			...
			DrawRoutines.drawXpRadioBorder(graphics,
					Theme.buttonBorderColor.getColor(), bx, by, w, h);
		}
