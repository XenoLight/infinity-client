...
public class TinyScrollButton extends BasicArrowButton {
	...
	/*
	 * ScrollButtonKey is used as key in the cache HashMap. Overrides equals()
	 * and hashCode().
	 */
	private static class ScrollButtonKey {
		...
		private ScrollButtonKey(final Dimension size, final boolean vertical, final Color c,
				final boolean pressed, final boolean enabled, final boolean rollover) {
			this.size = size;
		...
		public boolean equals(final Object o) {
			...
			final ScrollButtonKey other = (ScrollButtonKey) o;

			return size.equals(other.size) && vertical == other.vertical
			&& pressed == other.pressed && enabled == other.enabled
			&& rollover == other.rollover && c.equals(other.c);
		}

		@Override
		public int hashCode() {
			return size.hashCode() * c.hashCode() * (pressed ? 1 : 2)
	...
	private final TinyScrollBarUI scrollbarUI;
	 ...
	 *      boolean)
	 */
	public TinyScrollButton(final int direction, final TinyScrollBarUI scrollbarUI) {
		super(direction);
	...
	private void drawXpArrow(final Graphics g, final Dimension size) {
	...
	private void drawXpButton(final Graphics g, final Dimension size, Color c) {
		final boolean enabled = scrollbarUI.isThumbVisible();
		final boolean pressed = getModel().isPressed();
		final boolean rollover = getModel().isRollover()
		&& Theme.scrollRollover.getValue();
		final ScrollButtonKey key = new ScrollButtonKey(size,
				(direction == NORTH || direction == SOUTH), c, pressed,
				enabled, rollover);
		...
		final Image img = new BufferedImage(size.width, size.height,
		...
		case SwingConstants.SOUTH:
			...
			Color lightAlpha = new Color(c.getRed(), c.getGreen(), c.getBlue(),
		...
		//
		// // edges - blend borderColor with lightColor
		// imgGraphics.setColor(new Color(c.getRed(), c.getGreen(), c.getBlue(),
		// TinyScrollBarUI.ALPHA));
		// imgGraphics.drawLine(2, 1, 2, 1);
	...
	private void drawXpButtonNoCache(final Graphics g, final Dimension size, Color c) {
		final boolean enabled = scrollbarUI.isThumbVisible();
		final int h = size.height;
		final int w = size.width;
		int spread1 = Theme.scrollSpreadLight.getValue();
	...
	public Dimension getPreferredSize() {
		return new Dimension(Theme.scrollSize.getValue(),
				Theme.scrollSize.getValue());
	}
