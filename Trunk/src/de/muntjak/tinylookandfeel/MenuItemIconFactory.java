...
public class MenuItemIconFactory implements Serializable {
	...
	Serializable {
		...
		class RadioKey {
			...
			private final Color c, background;
			private final boolean selected;

			RadioKey(final Color c, final Color background, final boolean selected) {
				this.c = c;
			...
			public boolean equals(final Object o) {
				...
				final RadioKey other = (RadioKey) o;

				return selected == other.selected && c.equals(other.c)
				&& background.equals(other.background);
			}

			@Override
			public int hashCode() {
				return c.hashCode() * background.hashCode()
				* (selected ? 1 : 2);
			}
		}
		...
		public void paintIcon(final Component c, final Graphics g, final int x, final int y) {
			...
			final boolean isSelected = model.isSelected();
			final boolean isEnabled = model.isEnabled();
			final boolean isPressed = model.isPressed();
			final boolean isArmed = model.isArmed();
			b.getBackground();
			Color col = null;
		...
		private void paintIcon(final Graphics g, final Color c, final JMenuItem b,
			...
			final RadioKey key = new RadioKey(c, bg, selected);
			...
			final Image img = new BufferedImage(CHECK_ICON_SIZE.width,
					CHECK_ICON_SIZE.height, BufferedImage.TYPE_INT_ARGB);
			final Graphics imgGraphics = img.getGraphics();

			imgGraphics.setColor(bg);
			imgGraphics.fillRect(0, 0, CHECK_ICON_SIZE.width,
					CHECK_ICON_SIZE.height);
			...
			if (selected) {
				for (int row = 3; row < 7; row++) {
					for (int col = 3; col < 7; col++) {
						color = new Color(c.getRed(), c.getGreen(),
								c.getBlue(), 255 - ALPHA_CHECK[col][row]);
						imgGraphics.setColor(color);
						imgGraphics.drawLine(col, row, col, row);
					}
				}
			...
			if (TinyLookAndFeel.PRINT_CACHE_SIZES) {
				System.out.println("MenuItemIconFactory.cache.size="
						+ cache.size());
			}
		}

		private void paintIconNoCache(final Graphics g, final Color c, final boolean selected) {
			Color color;
	...
	private static class SystemMenuIcon implements Icon, UIResource,
	Serializable {

		private final int style;

		SystemMenuIcon(final int style) {
			this.style = style;
		}
