...
/**
 * TinyButtonUI. The UI delegate for JButton, JToggleButton, window buttons and
 * arrow buttons of JSpinner and JComboBox.
 * 
 * @version 1.4.0
 * @author Hans Bickel
 */
public class TinyButtonUI extends MetalButtonUI {
	...
	private static class ButtonKey {
		...
		private final boolean isBorderPainted;

		ButtonKey(final Color background, final Color parentBackground, final int height,
				final boolean rollover, final boolean isDefault, final boolean hasFocus,
				final boolean isBorderPainted) {
		...
		public boolean equals(final Object o) {
			...
			return height == other.height && rollover == other.rollover
		...
		@Override
		public int hashCode() {
			return background.hashCode() * parentBackground.hashCode() * height
			* (rollover ? 2 : 1) * (isDefault ? 8 : 4)
			* (hasFocus ? 16 : 8) * (isBorderPainted ? 32 : 16);
		}
	}
	...
	private void drawXpBorder(final AbstractButton b, final Graphics g, final int x, final int y,
			final int w, final int h) {
		final boolean isComboBoxButton = Boolean.TRUE.equals(b
				.getClientProperty("isComboBoxButton"));

		if (isComboBoxButton) {
			if (!b.isEnabled()) {
				DrawRoutines.drawRoundedBorder(g,
						Theme.comboBorderDisabledColor.getColor(), x, y, w, h);
			} else {
				DrawRoutines.drawRoundedBorder(g,
						Theme.comboBorderColor.getColor(), x, y, w, h);
				...
				if (b.getModel().isRollover() && Theme.comboRollover.getValue()) {
					DrawRoutines.drawRolloverBorder(g,
							Theme.buttonRolloverColor.getColor(), x, y, w, h);
				}
			}
		} else { // it's a JButton or a JSpinner button
			final boolean isDefault = (b instanceof JButton)
			&& (((JButton) b).isDefaultButton());
			final boolean isSpinnerButton = Boolean.TRUE.equals(b
					.getClientProperty("isSpinnerButton"));
			final boolean paintRolloverBorder = (isSpinnerButton && Theme.spinnerRollover
					.getValue())
					|| (!isSpinnerButton && Theme.buttonRolloverBorder
							.getValue());
			// New in 1.4.0: If isFocusPainted is false, no focus border
			// will be painted
			final boolean paintFocusBorder = isDefault
			|| (isSpinnerButton && Theme.buttonFocusBorder.getValue() && b
					.isFocusOwner())
					|| (!isSpinnerButton && Theme.buttonFocusBorder.getValue()
							&& b.isFocusOwner() && b.isFocusPainted());

			if (!b.isEnabled()) {
				DrawRoutines.drawRoundedBorder(g,
						Theme.buttonBorderDisabledColor.getColor(), x, y, w, h);
			} else {
				DrawRoutines.drawRoundedBorder(g,
						Theme.buttonBorderColor.getColor(), x, y, w, h);
				...
				if (b.getModel().isRollover() && paintRolloverBorder) {
					...
					DrawRoutines.drawRolloverBorder(g,
							Theme.buttonDefaultColor.getColor(), x, y, w, h);
				}
			}
		}
	}

	private void drawXpButton(final Graphics g, final AbstractButton b, final Color c,
		...
		final Color bg = b.getParent().getBackground();

		// With 1.4.0 added hasFocus to the key (bug only revealed with JDK 1.6)
		// and added isBorderPainted to the key.
		final ButtonKey key = new ButtonKey(c, bg, h, isRollover
				& Theme.buttonRolloverBorder.getValue(), isDefaultButton,
				b.hasFocus(), b.isBorderPainted());
		final Object value = cache.get(key);

		if (value != null) {
			// image already cached - paint image and return
			final int width = ((Image) value).getWidth(b);
			...
			int dx2 = sx2;
			final int dy2 = sy2;
			g.drawImage((Image) value, dx1, dy1, dx2, dy2, sx1, sy1, sx2, sy2,
					b);
		...
		if (b.isBorderPainted()) {
			...
			// Changed in 1.4.0: Moved the following block into these two
			// if-clauses
			// so additional pixels will be painted only if button has a Tiny
			// border.
			if (border != null
	...
	private void drawXpButtonNoCache(final Graphics g, final AbstractButton b, final Color c,
			final boolean isRollover) {
		if (!b.isContentAreaFilled())
			return;
		if (!b.isOpaque())
	...
	private void drawXpToolBarButton(final Graphics g, final AbstractButton b, final Color c,
	...
	public void installUI(final JComponent c) {
		...
		if (km != null) {
			// replace SPACE with ENTER (but SPACE will still work, don't know
			// why)
	...
	public void paint(final Graphics g, final JComponent c) {
		...
		if (isToolBarButton || isFileChooserButton) {
			...
			isDefaultButton = (c instanceof JButton)
			&& (((JButton) c).isDefaultButton());
			final boolean isRollover = button.getModel().isRollover();
			final boolean isDefinedBackground = c.getBackground().equals(
			...
			if (!button.isEnabled()) {
				...
				if (isRollover) {
					if (isDefinedBackground) {
						...
						col = ColorRoutines.darken(c.getBackground(),
								BG_CHANGE_AMOUNT);
					}
				} else {
				...
				if (isDefinedBackground) {
					...
					col = ColorRoutines.lighten(c.getBackground(),
							BG_CHANGE_AMOUNT);
				}
			} else {
				if (isDefinedBackground) {
	...
	protected void paintButtonPressed(final Graphics g, final AbstractButton button) {
		...
		if (!(button.getBackground() instanceof ColorUIResource)) {
			col = ColorRoutines
			.darken(button.getBackground(), BG_CHANGE_AMOUNT);
		...
		if (!(button instanceof JToggleButton)) {
			...
			if (Theme.shiftButtonText.getValue() && button.getText() != null
	...
	protected void paintFocus(final Graphics g, final AbstractButton b, final Rectangle viewRect,
		...
		final Rectangle focusRect = b.getBounds();
	...
	protected void paintToggleButtonIcon(final Graphics g, final JComponent c,
	...
	public void paintToolBarButton(final Graphics g, final AbstractButton b) {
		Color col = null;
	...
	public void update(final Graphics g, final JComponent c) {
		...
		isFileChooserButton = Boolean.TRUE
		.equals(c
