...
public class ImprovedRewardsBox extends Antiban {
	...
	private String settingsFile = "RewardBox.txt";
	Rectangle Temporary;
	String Filename;
	...
	private String[] primer = {
			"XP item", "Ore", "Coal", "Bars", "Gems", "Herbs", "Cash", 
			"Runes",  "Essence",  "Seeds", "Charms", "Surprise",
			"Emote", "Costume", "Drop",
			"Prayer", "Summoning", "Dungeoneering", "Runecrafting", "Attack",			
			"Strength", "Defence", "Ranged", "Magic", "Construction", "Hitpoints",
	...
	public int loop() {
		try  {
			if (iface.get(BoxIF).isValid()) {
				...
				String oldFname = new File(
						GlobalConfiguration.Paths.getSettingsDirectory())
				+ File.separator + "PRds";
				
				final File RewardsChoiceFile = new File(Filename);
				final File OldChoiceFile = new File(oldFname);
				
				// Rename the old PRds file to the new settings file name
				if (!RewardsChoiceFile.exists() && OldChoiceFile.exists())  {
					OldChoiceFile.renameTo(RewardsChoiceFile);
				}
				else if (!RewardsChoiceFile.exists())  {
					// Here, neither file exists so we'll just create it.
					final BufferedWriter out = new BufferedWriter(new FileWriter(Filename));	
					
					for (String entry: primer)  {
						out.write(entry);
						out.write("\n");
					}
				}
