...
+ "4. Start the bot</font>" + "</font>" + "</body>n" + "</htmln", summary = "")
public class fGrSmither extends Script implements PaintListener {

    private static final int bronze = 2349;
    private static final int iron = 2351;
    private static final int steel = 2353;
    private static final int mith = 2359;
    private static final int addy = 2361;
    private static final int rune = 2363;
    ...
    public final int energy = random(11, 90);
    ...
    public String bars;
    public int xpbar = 25;
    public final int[] tokeep = {2347, 2349, 2351, 2353, 2359, 2361, 2363};
    public final int startingLevel = skills.getRealLvl(Constants.STAT_SMITHING);
    int smithx = 0;
    int smithy = 0;
    int smithx2 = 0;
    int smithy2 = 0;
    ...
    public final RSTile boothTile = new RSTile(3189, 3435);
    public final RSTile smithTile = new RSTile(3187, 3424);
    ...
    public boolean clickInventoryItem(final int itemID) {
        ...
        final Point t = inventory.getItemPoint(idx);
        mouse.move(t, 5, 5);
        wait(random(50, 100));
        if (topTextContains("->")) {
            return true;
    ...
    public int gEnergy() {
        int a;
        try {
            a = Integer.parseInt(iface.getChild(750, 5).getText());
    ...
    public int loop() {
        if (iface.get(740).getChild(0).isValid()) {
            chooseIdle = 0;
            wait(random(300, 600));
            iface.clickChild(iface.get(740).getChild(3), "continue");
            level = skills.getRealLvl(Constants.STAT_SMITHING);
        }
        if (chooseIdle != 1) {
            if (awbest) {
                smithbest();
        ...
        if (inventory.getCount(hammer) == 1 && inventory.getCount(bar) >= needed
                && calculate.distanceTo(smithTile) > 5) {
            walk.to(smithTile);
            return random(236, 1026);
        }

        if (inventory.getCount(bar) <= needed && calculate.distanceTo(boothTile) < 5
                || inventory.getCount(hammer) != 1 && calculate.distanceTo(boothTile) < 5) {
            bank();
    ...
    public void onRepaint(final Graphics g) {
        ...
        if (bars2 == 1 || bars2 == 3 || bars2 == 5) {
            antall = skills.getXPToNextLvl(Constants.STAT_SMITHING) / xpbar
                    / needed * 2 + 2;
        }
        ...
        gainedExp = skills.getCurrentXP(Constants.STAT_SMITHING)
                - startingExp;
        g.setColor(BG);
        ...
        g.drawString(skills.getPercentToNextLvl(Constants.STAT_SMITHING)
                + "% to level " + (level + 1), 310, 255);
        g.drawString(
                "Gained "
                + (skills.getRealLvl(Constants.STAT_SMITHING) - startingLevel)
                + " levels", 310, 270);
    ...
    public boolean onStart(final Map<String, String> args) {
        ...
        level = startingLevel;
        startingExp = skills.getCurrentXP(Constants.STAT_SMITHING);
    ...
    public boolean openBank() {
        final RSObject bank = objects.getNearestByID(782);
        if (bank == null) {
            return false;
        }
        if (!tile.onScreen(bank.getLocation())) {
            camera.turnTo(bank.getLocation(), 15);
        }

        return tile.click(bank.getLocation(), "Use-quickly");
    ...
    public int smith() {
        ...
        if (smithIdle != 1) {
            ...
            wait(random(250, 500));
            final RSObject anvill = objects.getNearestByID(anvil);
            objects.at(anvill, "Use");
            wait(random(2500, 3000));
            if (iface.get(300).isValid() == false) {
                camera.setRotation(random(1, 359));
            }
            if (iface.get(300).isValid()) {
                if (toScroll == 1) {
                    mouse.click(random(492, 504), random(250, 300), true);
                    toScroll = 0;
                }
                mouse.click(random(smithx, smithx2), random(smithy, smithy2),
                        false);
                menu.action("All");
                smithIdle = 1;
            }
