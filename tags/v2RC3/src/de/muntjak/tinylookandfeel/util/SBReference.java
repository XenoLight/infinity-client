...
/**
 * SBReference describes a (mutable) color, specified by a reference color and
...
public class SBReference {
	...
	class ColorIcon implements Icon {
		...
		private final boolean paintGradients;

		ColorIcon(final boolean paintGradients) {
			this.paintGradients = paintGradients;
		}

		private void drawArrow(final Graphics g, final int x, final int y) {
		...
		public void paintIcon(final Component comp, final Graphics g, final int x, final int y) {
			final Color tempCol = g.getColor();
	...
	public static int getNumReferences(final int ref) {
		int n = 0;

		final Iterator ii = instances.iterator();
		while (ii.hasNext()) {
	...
	public static ColorUIResource getReferencedColor(final int ref) {
		switch (ref) {
		case MAIN_COLOR:
	...
	public static void loadDummyData(final DataInputStream in) throws IOException {
		try {
			...
			throw new IOException("SBReference.loadDummyData() : "
					+ ex.getMessage());
		}
	}
	public static void printReferences(final int ref) {
		final Iterator ii = instances.iterator();
		while (ii.hasNext()) {
			final SBReference sb = (SBReference) ii.next();
	...
	/**
	 * Constructor for font colors and super constructor for HSBReferences.
	 * SBReference will be added to instances vector.
	 * 
	 */
	public SBReference() {
	...
	/**
	 * Super constructor for HSBReference copy constructor. This SBReference
	 * will not be added to instances vector.
	 * 
	 * @param dummy
	 *            ignored
	 */
	public SBReference(final boolean dummy) {
		color = new ColorUIResource(Color.BLACK);
		ref = ABS_COLOR;
	}
	 ...
	 * @param ref
	 */
	public SBReference(final Color c, final int sat, final int bri, final int ref) {
	 ...
	 * @param locked
	 */
	public SBReference(final Color c, final int sat, final int bri, final int ref, final boolean locked) {
		color = new ColorUIResource(c);
		this.sat = sat;
		this.bri = bri;
		this.ref = ref;
	 ...
	 * @param other
	 */
	public SBReference(final SBReference other) {
	...
	/**
	 * Returns a copy of this SBReference and additionally stores the color
	 * reference (if any). Called if user copies parameters via Copy-Paste
	 * popup.
	 * 
	 * @return
	 */
	public SBReference copy() {
		final SBReference retVal = new SBReference(this);
	...
	/**
	 * If one of reference colors sub1 to sub8 has no references, returns its
	 * ref value, returns -1 if no empty reference color was found.
	 * 
	 * @return
	 */
	...
	private int getRefForColor(final ColorUIResource cr) {
	...
	public boolean isReferenceColor() {
		return locked || this.equals(Theme.sub1Color)
		|| this.equals(Theme.sub2Color) || this.equals(Theme.sub3Color)
		|| this.equals(Theme.sub4Color) || this.equals(Theme.sub5Color)
		|| this.equals(Theme.sub6Color) || this.equals(Theme.sub7Color)
	...
	public void load(final DataInputStream in) throws IOException {
		try {
			if (Theme.fileID >= Theme.FILE_ID_3A) {
				...
				color = new ColorUIResource(in.readInt(), in.readInt(),
						in.readInt());
			}
	...
	public void setBrightness(final int newBri) {
		bri = newBri;
	}
	...
	public void setLocked(final boolean newLocked) {
		locked = newLocked;
	}

	public void setReference(final int newRef) {
		ref = newRef;
	}

	public void setSaturation(final int newSat) {
		sat = newSat;
	}

	@Override
	public String toString() {
		return "SBReference[bri=" + bri + ",sat=" + sat + ",ref=" + ref
		+ ",c=(" + color.getRed() + "," + color.getGreen() + ","
		+ color.getBlue() + ")]";
	}
	...
	public void update(final Color c) {
		color = new ColorUIResource(c);
		sat = 0;
		bri = 0;
		ref = ABS_COLOR;
	...
	public void update(final Color c, final int sat, final int bri, final int ref) {
		color = new ColorUIResource(c);
		this.sat = sat;
		this.bri = bri;
		this.ref = ref;
	 ...
	 * @param cr
	 */
	public void update(final SBReference sb) {
	 ...
	 * @param cr
	 */
	private void update(final SBReference sb, final ColorUIResource cr) {
		...
		if (!sb.getReferenceColor().equals(cr)) {
			...
			if (newRef != -1) {
				ref = newRef;
				final SBControl control = ControlPanel.instance
				.getSBControlFromRef(ref);
	...
	/**
	 * Updates this SBReference from values of <code>other</code> argument. If
	 * the current reference color differs from the one stored in
	 * <code>referenceColors</code> vector, this SBReference will switch to
	 * absolute color mode.
	 * 
	 * @param other
	 * @param referenceColors
	 */
	public void update(final SBReference other, final Vector referenceColors) {
	...
	private void updateColor() {
		switch (ref) {
		case MAIN_COLOR:
			color = new ColorUIResource(ColorRoutines.getAdjustedColor(
