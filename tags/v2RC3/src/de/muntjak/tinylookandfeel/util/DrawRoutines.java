...
/**
 * DrawRoutines is a collection of static methods related to drawing.
 * 
 * @version 1.0
 * @author Hans Bickel
 */
public class DrawRoutines {
	...
	static {
		...
		graphicsConfiguration = ge.getDefaultScreenDevice()
		.getDefaultConfiguration();
	}

	public static ImageIcon colorizeIcon(final Image img, final HSBReference hsbRef) {
		final ColorRoutines nc = new ColorRoutines(hsbRef);

		// long t = System.nanoTime();
		final int w = img.getWidth(null);
		final int h = img.getHeight(null);

		final BufferedImage bufferedImg = graphicsConfiguration
		.createCompatibleImage(w, h, Transparency.TRANSLUCENT);

		final int[] pixels = new int[w * h];
		final PixelGrabber grabber = new PixelGrabber(img, 0, 0, w, h, pixels, 0, w);
		...
		if ((grabber.getStatus() & ImageObserver.ABORT) != 0) {
			...
			for (int y = 0; y < h; y++) {
				for (int x = 0; x < w; x++) {
					bufferedImg.setRGB(x, y,
							colorizePixel(pixels[y * w + x], nc));
				}
			}
	...
	private static int colorizePixel(final int px, final ColorRoutines nc) {
	...
	public static void drawEditableComboBorder(final Graphics g, final Color c, final int x,
	...
	public static synchronized void drawProgressBarBorder(final Graphics g, final Color c,
	 ...
	 * changed in 1.3
	 */
	public static void drawRolloverBorder(final Graphics g, final Color c, final int x, final int y,
	...
	public static void drawRolloverCheckBorder(final Graphics g, final Color c, final int x,
			final int y, final int w, final int h) {
		g.translate(x, y);
		Color color;
	...
	public static void drawRoundedBorder(final Graphics g, final Color c, final int x, final int y,
			final int w, final int h) {
		g.setColor(c);
		// rect
	...
	public static void drawSelectedXpTabBorder(final Graphics g, final Color c, int x,
			int y, int w, int h, final int tabPlacement) {
		final Color c2 = ColorRoutines.getAdjustedColor(
				Theme.tabRolloverColor.getColor(), 20, -30);
		g.setColor(c2);
		final Color c3 = ColorRoutines.getAverage(Theme.backColor.getColor(), c2);
	...
	public static void drawWindowButtonBorder(final Graphics g, final Color c, final int x,
			final int y, final int w, final int h) {
		g.setColor(c);
		// rect
	...
	public static void drawXpRadioBorder(final Graphics g, final Color c, final int x, final int y,
			final int w, final int h) {
		g.setColor(c);
	...
	public static void drawXpRadioRolloverBorder(final Graphics g, final Color c, final int x,
			final int y, final int w, final int h) {
		g.translate(x, y);
		Color color;
	...
	public static void drawXpTabBorder(final Graphics g, final Color c, int x, int y,
			int w, int h, final int tabPlacement) {
		Color c2 = null;
		g.setColor(c);
