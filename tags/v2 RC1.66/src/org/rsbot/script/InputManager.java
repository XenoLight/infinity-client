...
public class InputManager {
	...
	private static boolean isOnCanvas(final int x, final int y) {
		return (x > 0) && (x < Bot.getCanvas().getWidth()) && (y > 0)
		&& (y < Bot.getCanvas().getHeight());
	}
	...
	public void holdKey(final int keyCode, final int ms) {
		KeyEvent ke;
		ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED,
		...
		if (ms > 500) {
			...
			for (int i = 37; i < ms2; i += random(20, 40)) {
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED,
						System.currentTimeMillis() + i + 500, 0, keyCode,
						(char) keyCode);
				getClient().getKeyboard()._keyPressed(ke);
			}
		}
		final int delay2 = ms + random(-30, 30);
		ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED,
	...
	private void moveMouse(final int x, final int y) {
		...
		if (pressed) {
			final MouseEvent me = new MouseEvent(getTarget(),
					MouseEvent.MOUSE_DRAGGED, System.currentTimeMillis(), 0, x,
					y, 0, false);
		...
		if (!InputManager.isOnCanvas(x, y)) {
			final MouseEvent me = new MouseEvent(getTarget(),
					MouseEvent.MOUSE_EXITED, System.currentTimeMillis(), 0, x,
					y, 0, false);
			present = false;
			getClient().getMouse().sendEvent(me);
			return;
	 ...
	 */
	public void moveMouse(final int x, final int y, final int randomX,
			final int randomY) {
	...
	public void moveMouse(final int speed, final int x, final int y,
		...
		moveMouse(speed, x, y, randomX, randomY, false);
	}
	 ...
	 */
	public void moveMouse(final int speed, final int x, final int y,
			final int randomX, final int randomY, final boolean MousePaths) {
		...
		if (MousePaths) {
			final Point[] path = mouseHandler
			.generateMousePath((int) Math.hypot(thisX - x, thisX - y)
					/ 100 + random(1, 3), new Point(thisX, thisY),
					new Point(x, y));
			if (path == null) {
				new Exception(
			...
			for (int i = 1; i < path.length; i++) {
				try {
					if (i == path.length - 1) {
						windMouse(speed, path[i - 1].x, path[i - 1].y,
	...
	public void pressKey(final char ch) {
		KeyEvent ke;
		ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED,
				System.currentTimeMillis(), 0, ch, getKeyChar(ch));
		getClient().getKeyboard()._keyPressed(ke);
	}
	...
	private void releaseMouse(final int x, final int y, final boolean leftClick) {
		if (!pressed)
			return;
		MouseEvent me = new MouseEvent(getTarget(), MouseEvent.MOUSE_RELEASED,
		...
		if ((dragLength & 0xFF) <= 3) {
			me = new MouseEvent(getTarget(), MouseEvent.MOUSE_CLICKED,
					System.currentTimeMillis(), 0, x, y, 1, false,
					leftClick ? MouseEvent.BUTTON1 : MouseEvent.BUTTON3);
			getClient().getMouse().sendEvent(me);
		}
	...
	private void sendKey(final char ch, final int delay) {
		...
		if ((code == KeyEvent.VK_LEFT) || (code == KeyEvent.VK_UP)
			...
			if (!shift) {
				...
				getClient().getKeyboard()._keyPressed(ke);
				/* Event Typed */
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_TYPED,
						System.currentTimeMillis() + 0, 0, 0, ch, 0);
				...
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED,
						...
						InputEvent.SHIFT_DOWN_MASK, code, getKeyChar(ch),
				...
				final int delay2 = random(50, 120) + random(0, 100);
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED,
						System.currentTimeMillis() + delay2,
						InputEvent.SHIFT_DOWN_MASK, code, getKeyChar(ch),
						KeyEvent.KEY_LOCATION_STANDARD);
				getClient().getKeyboard()._keyReleased(ke);
	...
	public void sendKeys(final String text, final boolean pressEnter,
			final int minDelay, final int maxDelay) {
	 ...
	 */
	public void windMouse(final int curX, final int curY, final int targetX,
			final int targetY) {
		windMouse(MouseHandler.DEFAULT_MOUSE_SPEED, curX, curY, targetX,
				targetY);
	}
	 ...
	 */
	public void windMouse(final int speed, final int curX, final int curY,
