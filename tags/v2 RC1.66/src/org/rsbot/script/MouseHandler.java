...
public class MouseHandler {
	protected static final Logger logger = Logger.getLogger(MouseHandler.class.getName());
	...
	public static Point[] makeMouseSplinePath(final int Sx, final int Sy,
			...
			int CtrlVar) {
		Bot.debug(logger, "Moving from: " + Sx + ", " + Sy + " to: " + Ex
				+ ", " + Ey);
		...
		if (Ex - Sx != 0) {
			...
			for (i = 1; i <= pts; i++) {
				Controls.add(new Point(
						Math.round(Sx
								+ XInc
								* i
								+ (new java.util.Random().nextInt(CtrlVar * 2) - CtrlVar)),
								Math.round(slope
										* (Sx + XInc * i)
										+ b
										+ (new java.util.Random().nextInt(CtrlVar * 2) - CtrlVar))));
			...
			for (i = 1; i <= pts; i++) {
				Controls.add(new Point(new java.util.Random()
				.nextInt(CtrlVar * 2) - CtrlVar, Sy + (Ey - Sy) * i
				+ new java.util.Random().nextInt(CtrlVar * 2) - CtrlVar));
			}
		}
		...
		Bot.debug(logger, String.valueOf(1.0 / (dist / 5.0)));

		for (int d = 0; d < Controls.toArray().length; d++) {
			Bot.debug(logger, "X: " + Controls.get(d).x + ", Y:"
					+ Controls.get(d).y);
		}
	...
	public static Point[] makeSplinePath(final Point[] mp, final double ThetaInc) {
		...
		Bot.debug(logger, String.valueOf(ThetaInc));
		while (!done) {
			/* log.info((t); */
			if (t >= 1) {
				t = 1;
			...
			if ((temp.x != last.x) && (temp.y != last.y)) {
				...
				last.x = temp.x;
				last.y = temp.y;
				Bot.debug(logger, "this happened");
			}
			t = t + ThetaInc;
		}
		...
		try {
				...
				if (i == path.length - 1) {
					...
					path[i].setLocation(
							path[i - 1].x > destination.x ? MouseHandler.methods
									.random(destination.x, path[i - 1].x)
									: MouseHandler.methods.random(
											path[i - 1].x, destination.x),
											path[i - 1].y > destination.y ? MouseHandler.methods
													.random(destination.y, path[i - 1].y)
													: MouseHandler.methods.random(
															path[i - 1].y, destination.y));
				} else {
					path[i].setLocation(
			...
			for (final Point element : path) {
				...
				for (int a = 0; a < unsorted.size(); a++) {
					if ((int) Math.hypot(
							unsorted.get(a).getX() - destination.getX(),
							unsorted.get(a).getY() - destination.getY()) >= dist) {
	...
	public void moveMouse(final int speed, final int x1, final int y1,
		...
		if ((x2 == -1) && (y2 == -1))
			/*
			 * MouseHandler.log
			 * .warning("Non-fatal error. Please post log on forums. (" + x2 +
			 * "," + y2 + ")");
			 */
			return;
		if (randX <= 0) {
		...
		try {
			...
			final long timeToMove = MouseHandler.fittsLaw(
