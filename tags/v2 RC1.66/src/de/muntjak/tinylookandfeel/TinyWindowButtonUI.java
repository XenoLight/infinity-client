...
public class TinyWindowButtonUI extends TinyButtonUI {
	...
	private static class ButtonKey {
		...
		private final boolean frameSelected;
		private final boolean frameMaximized;

		ButtonKey(final Color background, final int size, final int type, final boolean rollover,
				final boolean pressed, final boolean frameSelected, final boolean frameMaximized) {
		...
		public boolean equals(final Object o) {
			...
			&& rollover == other.rollover && pressed == other.pressed
		...
		@Override
		public int hashCode() {
			return background.hashCode() * type * size * (rollover ? 2 : 1)
			* (pressed ? 8 : 4) * (frameSelected ? 32 : 16)
	...
	protected final static Dimension frameExternalButtonSize = new Dimension(
			21, 21);
	protected final static Dimension frameInternalButtonSize = new Dimension(
			17, 17);

	protected final static Dimension framePaletteButtonSize = new Dimension(13,
			13);
	/**
	 ...
	 */
	public static TinyWindowButtonUI createButtonUIForType(final int type) {
	...
	private void drawXpButton(final Graphics g, final AbstractButton button, final Color c,
			final int w, final int h, final boolean frameSelected, final boolean frameMaximized) {
		// We have 3 button sizes:
		// 21x21 px. for frames and dialogs
		// 17x17 px. for internal frames
		// 13x13 px. for internal palettes
		final ButtonKey key = new ButtonKey(c, w, type, (button.getModel()
				.isRollover() || button.getModel().isArmed()), button
				.getModel().isPressed(), frameSelected, frameMaximized);
		...
		if (button.getClientProperty(EXTERNAL_FRAME_BUTTON_KEY) == Boolean.TRUE) {
			...
			if (frameSelected) {
				...
				imgGraphics
				.setColor(TinyInternalFrameBorder.disabledUpperColor);
				imgGraphics.drawLine(0, 0, w - 1, 0);
				imgGraphics.drawLine(0, 1, 0, 1); // ol
				imgGraphics.drawLine(w - 1, 1, w - 1, 1); // or
				imgGraphics
				.setColor(TinyInternalFrameBorder.disabledLowerColor);
				imgGraphics.drawLine(0, h - 1, w - 1, h - 1);
				imgGraphics.drawLine(0, h - 2, 0, h - 2); // ul
				imgGraphics.drawLine(w - 1, h - 2, w - 1, h - 2); // ur
			}
			...
			if (!button.isEnabled()) {
				...
				if (type == CLOSE) {
					col = ColorRoutines.darken(
							Theme.frameButtCloseColor.getColor(), 20);
				} else {
					col = ColorRoutines.darken(Theme.frameButtColor.getColor(),
							20);
				}
			}
		...
		if (!button.isEnabled()) {
			if (type == CLOSE) {
				imgGraphics.setColor(Theme.frameSymbolCloseDisabledColor
						.getColor());
			} else {
		...
		drawXpSymbol(imgGraphics, button, c, w, h, frameSelected,
				frameMaximized);
	...
	private void drawXpButtonNoCache(final Graphics g, final AbstractButton button,
			final Color c, final int w, final int h, final boolean frameSelected, final boolean frameMaximized) {
		if (button.getClientProperty(EXTERNAL_FRAME_BUTTON_KEY) == Boolean.TRUE) {
	...
	private void drawXpLargeButton(final Graphics g, final AbstractButton b, final Color c,
			final int w, final int h, final boolean frameSelected) {
		g.drawLine(1, 2, 1, h - 2);
		g.drawLine(1, 1, w - 2, 1);
		g.drawLine(w - 2, h - 2, w - 2, h - 2);

		final boolean isDisabledButton = b
		.getClientProperty(TinyWindowButtonUI.DISABLED_WINDOW_BUTTON_KEY) == Boolean.TRUE;
		...
		final float spreadStep1 = 10.0f * spread1 / (h - 5);
		final float spreadStep2 = 10.0f * spread2 / (h - 5);
		final int halfY = h / 2;
		int yd;
		...
		if (!b.isEnabled()) {
			if (type == CLOSE) {
				DrawRoutines.drawWindowButtonBorder(g,
						Theme.frameButtCloseBorderDisabledColor.getColor(), 0,
						0, w, h);
			} else {
				DrawRoutines.drawWindowButtonBorder(g,
						Theme.frameButtBorderDisabledColor.getColor(), 0, 0, w,
						h);
			}
		} else {
			if (type == CLOSE) {
	...
	private void drawXpLargeSymbol(final Graphics g, final AbstractButton b, final Color c,
			final int w, final int h, final boolean frameSelected, final boolean frameMaximized) {
		if (!frameSelected) {
			if (b.isEnabled() && !b.getModel().isPressed()) {
				...
				if (type == CLOSE) {
					if (b.getModel().isPressed()) {
						g.setColor(Theme.frameSymbolClosePressedColor
								.getColor());
					} else {
						g.setColor(Theme.frameSymbolCloseDisabledColor
								.getColor());
					}
				} else {
					if (b.getModel().isPressed()) {
	...
	private void drawXpSmallSymbol(final Graphics g, final AbstractButton button, final Color c,
			final int w, final int h, final boolean frameSelected, final boolean frameMaximized) {
		if (!frameSelected) {
			...
			if (button.getModel().isPressed()
					&& (button.getModel().isRollover() || button.getModel()
							.isArmed())) {
				if (type == CLOSE) {
					g.setColor(Theme.frameSymbolClosePressedColor.getColor());
				} else {
	...
	private void drawXpSymbol(final Graphics g, final AbstractButton button, final Color c,
			final int w, final int h, final boolean frameSelected, final boolean frameMaximized) {
		if (button.getClientProperty(EXTERNAL_FRAME_BUTTON_KEY) == Boolean.TRUE) {
			// we are a frame
		...
		if (button.getParent() instanceof TinyInternalFrameTitlePane) {
			if (((TinyInternalFrameTitlePane) button.getParent()).isPalette()) {
				...
				drawXpSmallSymbol(g, button, c, w, h, frameSelected,
						frameMaximized);
				return;
			}
		}
	...
	public void paint(final Graphics g, final JComponent c) {
		...
		final Container window = c.getParent();

		if (window instanceof TinyInternalFrameTitlePane) {
			frameSelected = ((TinyInternalFrameTitlePane) window)
			.isFrameSelected();
			frameMaximized = ((TinyInternalFrameTitlePane) window)
			.isFrameMaximized();
		...
		if (TinyLookAndFeel.controlPanelInstantiated) {
			drawXpButtonNoCache(g, button, col, w, h, frameSelected,
					frameMaximized);
		} else {
