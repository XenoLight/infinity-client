...
public class TinyScrollBarUI extends BasicScrollBarUI {
	...
	/**
	 * Basically does BasicScrollBarUI.TrackListener the right job, it just
	 * needs an additional repaint and rollover management
	 */
	...
	protected class OrientationChangeListener implements PropertyChangeListener {
		...
		public void propertyChange(final PropertyChangeEvent e) {
			final Integer orient = (Integer) e.getNewValue();

			if (scrollbar.getComponentOrientation().isLeftToRight()) {
				if (incrButton instanceof TinyScrollButton) {
					((TinyScrollButton) incrButton).setDirection(orient
							.intValue() == HORIZONTAL ? EAST : SOUTH);
				}
				if (decrButton instanceof TinyScrollButton) {
					((TinyScrollButton) decrButton).setDirection(orient
	...
	/*
	 * ThumbKey is used as key in the cache HashMap. Overrides equals() and
	 * hashCode().
	 */
	private static class ThumbKey {

		private final Color c;
		private final int spread1;
		private final int spread2;
		private final Dimension size;
		private final boolean vertical;

		ThumbKey(final Color c, final int spread1, final int spread2, final boolean vertical,
		...
		public boolean equals(final Object o) {
			...
			final ThumbKey other = (ThumbKey) o;

			return spread1 == other.spread1 && spread2 == other.spread2
			&& vertical == other.vertical && size.equals(other.size)
			&& c.equals(other.c);
		}

		@Override
		public int hashCode() {
			return c.hashCode() * size.hashCode() * (vertical ? 1 : 2)
			* spread1 * spread2;
		}
	}
	...
	private void calculateButtonsEnabled() {
		final float extent = scrollbar.getVisibleAmount();
		final float range = scrollbar.getMaximum() - scrollbar.getMinimum();
	...
	private void drawXpThumb(final Graphics g, final JComponent comp, final Rectangle t) {
		final int spread1 = Theme.scrollSpreadLight.getValue();
		final int spread2 = Theme.scrollSpreadDark.getValue();
		Color c = null;
		...
		final ThumbKey key = new ThumbKey(c, spread1, spread2,
		...
		final Image img = new BufferedImage(t.width, t.height,
		...
		int x2 = t.width - 1;
		int y2 = t.height - 1;
		final float spreadStep1 = 10.0f * spread1 / 10;
		final float spreadStep2 = 10.0f * spread2 / 10;
		...
		case Adjustable.VERTICAL:
			...
			imgGraphics.drawLine(x1 + t.width - 2, y1 + 2, x1 + t.width - 2,
					y2 - 2);
			...
			imgGraphics.setColor(new Color(a.getRed(), a.getGreen(), a
					.getBlue(), ALPHA));
			imgGraphics.drawLine(x1 + 2, y1 + 1, x1 + 2, y1 + 1);
			imgGraphics.drawLine(x1 + t.width - 2, y1 + 1, x1 + t.width - 2,
					y1 + 1);
			imgGraphics.drawLine(x1 + 2, y2 - 1, x1 + 2, y2 - 1);
			imgGraphics.drawLine(x1 + t.width - 2, y2 - 1, x1 + t.width - 2,
					y2 - 1);
		...
		case Adjustable.HORIZONTAL:
			...
			imgGraphics.setColor(Theme.scrollBorderLightColor.getColor());
			imgGraphics.drawLine(x1 + 2, y1 + t.height - 2, x2 - 2, y1
					+ t.height - 2);
			...
			imgGraphics.drawLine(x1 + 1, y1 + t.height - 2, x1 + 1, y1
					+ t.height - 2);
			imgGraphics.drawLine(x2 - 1, y1 + 2, x2 - 1, y1 + 2);
			imgGraphics.drawLine(x2 - 1, y1 + t.height - 2, x2 - 1, y1
					+ t.height - 2);
		...
		// if(y < halfY) {
		// yd = halfY - y;
		// imgGraphics.setColor(ColorRoutines.lighten(c, (int)(yd *
		// spreadStep1)));
		// }
		// else if(y == halfY) {
		// imgGraphics.setColor(c);
		...
		// else {
		// yd = y - halfY;
		// imgGraphics.setColor(ColorRoutines.darken(c, (int)(yd *
		// spreadStep2)));
		// }
		//
		...
		// // edges - blend borderColor with lightColor
		// Color a = Theme.scrollBorderColor.getColor();
		// imgGraphics.setColor(new Color(a.getRed(), a.getGreen(), a.getBlue(),
		// alpha));
	...
	private void drawXpThumbNoCache(final Graphics g, final JComponent comp, final Rectangle t) {
		Color c = null;
		...
		case Adjustable.VERTICAL:
			...
			for (int y = 1; y < h; y++) {
				if (y < halfY) {
					yd = halfY - y;
					g.setColor(ColorRoutines.lighten(c,
							(int) (yd * spreadStep1)));
				} else if (y == halfY) {
					g.setColor(c);
				} else {
	...
	private void drawXpTrack(final Graphics g, final Rectangle t) {
		if (isThumbVisible()) {
		...
		if (scrollbar.getOrientation() == Adjustable.VERTICAL) {
			...
			g.drawLine(t.x, t.y + t.height - 1, t.x + t.width - 1, t.y
					+ t.height - 1);
		}
	}
	...
	/**
	 * A return value of <code>true</code> signals that scrollbar buttons shall
	 * be painted enabled whereas a return value of <code>false</code> signals
	 * that scrollbar buttons shall be painted disabled.
	 * 
	 * @return <code>true</code> if thumb is visible, <code>false</code>
	 *         otherwise.
	 */
