...
public class TinyLookAndFeel extends MetalLookAndFeel {
	...
	/**
	 * If <code>true</code>, each drawing cache reports its size as new entries
	 * are added. Should be false for production build.
	 */
	...
	/**
	 * A global <code>Robot</code> instance used for the ControlPanel magnifier,
	 * to antialias frame caption edges and to capture backgrounds for popup
	 * menu shadows.
	 */
	...
	/**
	 * Clears all caches used to speed up drawing of components. This method is
	 * called automatically if new themes are loaded.
	 * 
	 */
	 ...
	 *         &quot;Tree.closedIcon&quot;
	 */
	public static String getSystemIconName(final int index) {
		switch (index) {
		case 0:
	 ...
	 * @return an icon
	 */
	public static Icon getUncolorizedSystemIcon(final int index) {
		switch (index) {
		case 0:
	 ...
	 * @return the specified icon or <code>null</code> if the icon resource
	 *         doesn't exist.
	 */
	public static ImageIcon loadIcon(final String fileName) {
		...
		if (fileName.indexOf("/") != -1) {
			...
			.getResource("de/muntjak/tinylookandfeel/" + fileName);
		} else {
			url = Thread
			.currentThread()
		...
		if (url == null) {
			...
			if (fileName.indexOf("/") != -1) {
				...
				url = TinyLookAndFeel.class
				.getResource("/de/muntjak/tinylookandfeel/icons/"
						+ fileName);
			}

			if (url == null) {
				System.err
				.println("TinyLaF: Icon directory could not be resolved."
	...
	/**
	 * Creates the default theme and installs it. The TinyDefaultTheme is used
	 * as default.
	 */
	...
	protected void initClassDefaults(final UIDefaults table) {
		...
		table.putDefaults(new Object[] {
				...
				"CheckBoxUI",
				"de.muntjak.tinylookandfeel.TinyCheckBoxUI",
				"TextFieldUI",
				"de.muntjak.tinylookandfeel.TinyTextFieldUI",
				"TextAreaUI",
				...
				// properties were not propagated, introduced
				// TinyFormattedTextFieldUI
				// in v1.4.0
				"FormattedTextFieldUI",
				"de.muntjak.tinylookandfeel.TinyFormattedTextFieldUI",
				"PasswordFieldUI",
				"de.muntjak.tinylookandfeel.TinyPasswordFieldUI",
				...
				"ToolBarSeparatorUI",
				"de.muntjak.tinylookandfeel.TinyToolBarSeparatorUI",
				...
				"de.muntjak.tinylookandfeel.TinyRadioButtonMenuItemUI",
				...
				"ProgressBarUI",
				"de.muntjak.tinylookandfeel.TinyProgressBarUI",
				"InternalFrameUI",
				"de.muntjak.tinylookandfeel.TinyInternalFrameUI",
				"RadioButtonUI",
				"de.muntjak.tinylookandfeel.TinyRadioButtonUI", "ComboBoxUI",
				"de.muntjak.tinylookandfeel.TinyComboBoxUI",
				"PopupMenuSeparatorUI",
				"de.muntjak.tinylookandfeel.TinyPopupMenuSeparatorUI",
	...
	protected void initComponentDefaults(final UIDefaults table) {
		...
		final Border textFieldBorder = new TinyTextFieldBorder();
		...
		table.put("CheckBoxMenuItem.checkIcon",
				MenuItemIconFactory.getCheckBoxMenuItemIcon());
		table.put("RadioButtonMenuItem.checkIcon",
				MenuItemIconFactory.getRadioButtonMenuItemIcon());
		...
		table.put("FileChooser.upFolderIcon",
				loadIcon("ParentDirectoryIcon.png"));
	...
	private void searchDefaultTheme() {
		...
		if (Theme.loadTheme(defaultURL)) {
			...
			if (Theme.loadTheme(defaultURL)) {
				...
				try {
					defaultURL = new File(
							TinyUtils.getSystemProperty("user.home"),
					...
					if (Theme.loadTheme(defaultURL)) {
						...
						defaultURL = new File(
								TinyUtils.getSystemProperty("user.dir"),
								Theme.DEFAULT_THEME).toURI().toURL();
				...
				// with Java Web Start
				catch (final AccessControlException ignore) {
				}
			}
		}

		final String info = "TinyLaF v" + VERSION_STRING + "\n";
		if (loadedFrom == null) {
