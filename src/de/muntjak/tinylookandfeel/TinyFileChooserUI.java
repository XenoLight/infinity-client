...
public class TinyFileChooserUI extends MetalFileChooserUI {
	...
	private TinyFileChooserUI(final JFileChooser c) {
		super(c);
	}
	...
	public void installComponents(final JFileChooser fc) {
		...
		final Component[] ci = fc.getComponents();

		for (int i = 0; i < ci.length; i++) {
			if (ci[i] instanceof JPanel) {
				final Component[] cj = ((Container) ci[i]).getComponents();

				for (int j = 0; j < cj.length; j++) {
					if (cj[j] instanceof JPanel) {
						final Component[] ck = ((Container) cj[j]).getComponents();
						final Vector buttons = new Vector();

						for (int k = 0; k < ck.length; k++) {
							if (ck[k] instanceof AbstractButton) {
								final Icon icon = ((AbstractButton) ck[k]).getIcon();
						...
						if (buttons.size() >= 4) {
							...
							final JPanel topButtonPanel = (JPanel) cj[j];
							final Border toolButtonBorder = new TinyToolButtonBorder();

							// we want other gaps between button
							topButtonPanel.removeAll();

							// order of buttons should already be left to right
							final Iterator ii = buttons.iterator();
							while (ii.hasNext()) {
								final AbstractButton b = (AbstractButton) ii.next();

								b.putClientProperty(IS_FILE_CHOOSER_BUTTON_KEY,
										Boolean.TRUE);
								b.setOpaque(false);
								b.setBorder(toolButtonBorder);

								if (b instanceof JToggleButton) {
									topButtonPanel.add(Box
											.createRigidArea(hstrut1));
	...
	private boolean isFileChooserIcon(final Icon icon) {
		if (icon == null)
			return false;

		return icon.equals(upFolderIcon) || icon.equals(homeFolderIcon)
		|| icon.equals(newFolderIcon) || icon.equals(listViewIcon)
