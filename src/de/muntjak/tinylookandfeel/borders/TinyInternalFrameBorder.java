...
UIResource {
	...
	private void drawXpBorder(final Graphics g, final int x, final int y, final int w, final int h) {
		// left
	...
	private void drawXpCaption(final Graphics g, final int x, final int y, final int w, final int h, final Color c) {
		...
		final CaptionKey key = new CaptionKey(isActive, titleHeight);
		...
		if (value != null) {
			...
			g.drawImage((Image) value, x + 6, y, x + w - 6, y + 5, 0, 0, 1, 5,
					frame);
			g.drawImage((Image) value, x + 1, y + 5, x + w - 1,
					y + titleHeight, 0, 5, 1, titleHeight, frame);
		...
		if (TinyLookAndFeel.PRINT_CACHE_SIZES) {
			System.out.println("TinyInternalFrameBorder.cache.size="
					+ cache.size());
		}
	}

	private void drawXpCaptionNoCache(final Graphics g, final int x, final int y, final int w, final int h,
			final Color c) {
		if (isPalette) {
	...
	private void drawXpPaletteCaption(final Graphics g, final int x, final int y, final int w, final int h,
			final Color c) {
		int y2 = y;
		int spread1 = Theme.frameSpreadDarkDisabled.getValue();
		int spread2 = Theme.frameSpreadLightDisabled.getValue();
	...
	private void drawXpPaletteCaptionNoCache(final Graphics g, final int x, final int y, final int w,
			final int h, final Color c) {
		int y2 = y;
		int spread1 = Theme.frameSpreadDarkDisabled.getValue();
		int spread2 = Theme.frameSpreadLightDisabled.getValue();
		Color borderColor = null;
	...
	public Insets getBorderInsets(final Component c) {
		...
		return new Insets(0, TinyFrameBorder.FRAME_BORDER_WIDTH,
