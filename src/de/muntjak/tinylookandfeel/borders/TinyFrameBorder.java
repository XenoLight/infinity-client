...
public class TinyFrameBorder extends AbstractBorder implements UIResource {
	...
	private void drawXpBorder(final Graphics g, final int x, final int y, final int w, final int h) {
		...
		if (TinyLookAndFeel.ROBOT != null) {
			...
			final int wy = window.getLocationOnScreen().y;

			theRect.setBounds(wx, wy, 4, 4);
			g.drawImage(TinyLookAndFeel.ROBOT.createScreenCapture(theRect), x,
					y, null);
	...
	public Insets getBorderInsets(final Component c) {
		...
		return new Insets(0, FRAME_BORDER_WIDTH, FRAME_BORDER_WIDTH,
				FRAME_BORDER_WIDTH);
	}
