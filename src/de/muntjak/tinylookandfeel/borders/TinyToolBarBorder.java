...
public class TinyToolBarBorder extends ToolBarBorder {
	...
	protected void drawXPBorder(final Component c, final Graphics g, final int x, final int y, final int w,
			final int h) {
		g.translate(x, y);
		...
		if (((JToolBar) c).isFloatable()) {
			...
			if (((JToolBar) c).getOrientation() == HORIZONTAL) {
				...
				if (!c.getComponentOrientation().isLeftToRight()) {
					xoff = c.getBounds().width
					- TinyToolBarUI.FLOATABLE_GRIP_SIZE + 3;
				}
