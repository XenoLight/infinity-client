...
public class TinyUtils {
	...
	/*
	 * The value for this key is evaluated at drawString(JComponent, Graphics2D,
	 * String, int, int).
	 */
	/**
	 * This key is a hidden "feature". Setting this client property (for any
	 * tabbed pane, menu or menu item) to <code>Boolean.TRUE</code> overrides
	 * all other text antialiasing settings.
	 */
	public static final Object AA_TEXT_PROPERTY_KEY = new StringBuffer(
	"TinyAATextPropertyKey");

	static {
		javaVersion = getSystemPropertyPrivileged("java.version");

		if (javaVersion != null) {
			is1dot4 = javaVersion.startsWith("1.0")
		...
		// (see drawString(...)).
		// Note: osSettings will be non-null with 1.6 or higher JREs and
		// provided
		...
		osSettings = (Map) (Toolkit.getDefaultToolkit()
				.getDesktopProperty("awt.font.desktophints"));

		Toolkit.getDefaultToolkit().addPropertyChangeListener(
				"awt.font.desktophints", new PropertyChangeListener() {
					@Override
	 ...
	 */
	private static void drawString(final JComponent client, final Graphics2D g2d,
			final String text, final int x, final int y) {
		if (client != null
				&& Boolean.TRUE.equals(client
						.getClientProperty(AA_TEXT_PROPERTY_KEY))) {
			// paint antialiased
			final Object oldTA = g2d
		...
		if (osSettings != null) {
			final Map oldHints = getRenderingHints(g2d, osSettings, SAVED_HINTS);
	...
	/*
	 * Called for JMenu, all kinds of menu items and JTabbedPane. (Because we
	 * need to set the text color for those components we cannot use the default
	 ...
	 */
	public static void drawStringUnderlineCharAt(final JComponent client, final Graphics g,
		...
		if (underlinedIndex >= 0 && underlinedIndex < text.length()) {
			...
			final int underlineRectX = x
			+ fm.stringWidth(text.substring(0, underlinedIndex));
			final int underlineRectY = y;
			final int underlineRectWidth = fm.charWidth(text.charAt(underlinedIndex));
	 ...
	 */
	private static RenderingHints getRenderingHints(final Graphics2D g2d,
			final Map hintsToSave, RenderingHints savedHints) {
		if (savedHints == null) {
		...
		final Iterator ii = hintsToSave.keySet().iterator();
		while (ii.hasNext()) {
			final RenderingHints.Key key = (RenderingHints.Key) ii.next();
	...
	/**
	 * Returns the value of the system property corresponding to
	 * <code>key</code> argument.
	 * 
	 * @param key
	 *            a non-null system property key
	 * @return the value of a system property, null if either the key is unknown
	 *         or a SecurityExcpetion was raised
	 */
	...
	/**
	 * Returns <code>true</code> if we are running JRE 1.5.x, <code>false</code>
	 * otherwise.
	 * 
	 * @return <code>true</code> if we are running JRE 1.5.x, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Returns <code>true</code> if we are running JRE 1.6.x, <code>false</code>
	 * otherwise.
	 * 
	 * @return <code>true</code> if we are running JRE 1.6.x, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Returns true if we are running on Linux, false otherwise.
	 * 
	...
	/**
	 * Returns true if we are running on Mac OS, false otherwise.
	 * 
	 * @return true if we are running on Mac OS, false otherwise.
	 */
