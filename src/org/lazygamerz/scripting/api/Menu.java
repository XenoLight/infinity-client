...
public class Menu {
	...
	public boolean action(final String action, final String option) {
		...
		return clickAction(action, option);
	}
	 ...
	 * Note: This method fails when the index is not related to the first
	 * collapsed entry.
	 * 
	 * @param i
	...
	/**
	 * Left clicks the item having the specified action string.
	 * 
	 * @param action 
	 *            The action to be selected            
	 * @return <tt>true</tt> if the mouse was clicked; otherwise <tt>false</tt>.
	 */
	public boolean clickAction(final String action) {
		return clickAction(action, null);
	}
	
	/**
	 * Left clicks the item having the specified action and option strings.
	 * 
	 * @param action 
	 *            The action to be selected   
	 * @param option 
	 ...
	 * @return <tt>true</tt> if the mouse was clicked; otherwise <tt>false</tt>.
	 */
	public boolean clickAction(final String action, final String option) {
		if (!isOpen()) {
			return false;
		}
		...
		// Setup for case-insensitive check for items containing the specified
		// action+option.
		...
		if (option==null)  {
			targetItem = action.toLowerCase();
		}
		else {
			targetItem = 
				String.format("%s %s", action.toLowerCase(), option.toLowerCase());
		}
		
		boolean containsAction = false;
		for (String item  : itemsList)  {
			if (item.contains(targetItem))  {
				containsAction=true;
				break;
			}
		}
		...
		// Even the collapsed menus are sensitive to the ordering of
		...
		if (isCollapsed()) {
			...
			return clickMain(items, getIndex(action));
		}
	}
